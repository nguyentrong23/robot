...
void PnpController::PnpControllerStop() {
  ...
  dh_controller_->DH_AddFuncToQueue(
      DH_PGC::SetGripperPosition(rgi_address_, rgi_grip_open_pos_));
...
void PnpController::ReceivedNewFrame(cv::Mat frame) {
  ...
  is_found_2_objects_ =
      (image_matcher_->matchingResult.Objects.size() >= NUMBER_OBJECTS_FOUND) ? true : false;
...
/*      bool confirm_flag = true;*/
      ...
      break;
    case PnpDevicePin::kPnpPin_PlcClampConfirm:
        // in case auxiliary relay not in device map, it seem config flag ignore
        fx_plc_->GetAuxiliaryRelayState(m_device_clamp_confirm_, confirm_clamp_flag);
        if (confirm_clamp_flag) {
            emit PnpSignal_HasNewMessage("[PNP Controller]: PLC confirm signal is set.");
            robot_->SetContinueTrigger();
            SetPnpMoveState(PnpMoveState::kPnpMovePlacing);
        } else {
            SetPnpMoveState(PnpMoveState::kPnpMoveWaitPlcClampConfirm);
        }
        break;
...
void PnpController::PlcMDeviceChanged(int number, bool value) {
  ...
  } else if ((number == m_device_clamp_confirm_) && value) {
      if (move_state_current_ == PnpMoveState::kPnpMoveWaitPlcClampConfirm) {
          robot_->SetContinueTrigger();
          SetPnpMoveState(kPnpMovePlacing);
      }
  }
...
void PnpController::MoveToStandby() {
  ...
  robot_->pushCommand(HansCommand::SetOverride(0, SET_OVERIDE));
...
//           << "Rotate angle:" << object_rotate_angle_;
  ...
  robot_->pushCommand(
      HansCommand::WayPointLRelRef(0, pick_point, 0, 0, PICK_POINT_HIGH_OFFSET, 0,
                                   veloc_low_, accel_low_, 10));
  ...
  if (object_rotate_angle_ != 0) {
    ...
    robot_->pushCommand(
        HansCommand::WayPointLRelRef(0, position_rotate_, 0, ROTATE_POINT_Y_OFFSET_PUTIN, ROTATE_POINT_HIGH_OFFSET, 0,
                                     veloc_fast_, accel_fast_, 10));
 /*
Insert wait LCE confirm move done
*/

    robot_->pushCommand(
        HansCommand::WayPointLRelRef(0, position_rotate_, 0, ROTATE_POINT_Y_OFFSET_PUTIN, 0, 0,
                                     veloc_fast_, accel_fast_, 10));
    robot_->pushCommand(
    ...
    robot_->pushCommand(
        HansCommand::WayPointLRelRef(0, position_rotate_, 0, 0, ROTATE_POINT_HIGH_OFFSET, 0,
...
/*
Robot move picking tiếp cái khác
*/


 /*
Gửi tín hiệu cho LCE chạy
*/
...
/*
    ...
    robot_->pushCommand(
        HansCommand::WayPointL(0, position_rotate_, veloc_low_, accel_low_, 10));
*/
    /*pick object from +Y rotate position*/
    robot_->pushCommand(
        HansCommand::WayPointLRelRef(0, position_rotate_, 0, ROTATE_POINT_Y_OFFSET_PUTOUT, 0, 0,
                                     veloc_fast_, accel_fast_, 10));
...
/*
gửi tín hiệu tới PLC là đã đến
*/
/*
đợi PLC phản hồi
*/
...
/*
  ...
  robot_->pushCommand(
      HansCommand::WayPointLRelRef(0, place_point, 0, 0, 250, 0,
                                   veloc_fast_, accel_fast_, 10));
*/
 /*go to place HIGH X+50 position*/
  robot_->pushCommand(
      HansCommand::WayPointLRelRef(0, place_point, PLACE_POINT_X_OFFSET, 0, PLACE_POINT_HIGH_OFFSET, 0,
                                   veloc_fast_, accel_fast_, 10));
  ...
  /*go to place X+50 */
  robot_->pushCommand(
      HansCommand::WayPointLRelRef(0, place_point, PLACE_POINT_X_OFFSET, 0, 0, 0,
                                   veloc_fast_, accel_fast_, 10));
  ...
  /*wait plc place confirm when place to plc*/
  // wait plc place confirm
  robot_->pushCommand(
      HansCommand::TriggerOutputInt(static_cast<int>(kPnpPin_PlcClampConfirm)));
  robot_->pushCommand(
      HansCommand::WaitContinueTrigger());
...
*/
    ...
    robot_->pushCommand(
        HansCommand::WayPointLRelRef(0, place_point, PLACE_POINT_X_OFFSET, 0, 0, 0,
                                     veloc_fast_, accel_fast_, 10));
    /*move to place HIGH X+ position*/
    robot_->pushCommand(
        HansCommand::WayPointLRelRef(0, place_point, PLACE_POINT_X_OFFSET, 0, PLACE_POINT_HIGH_OFFSET, 90,
                                     veloc_fast_, accel_fast_, 10));
...
int PnpController::getTotalObjectPicked(){
    return sum_picked_objects_;
}

void PnpController::setTotalObjectPicked(int value) {
    sum_picked_objects_ = value;
}
