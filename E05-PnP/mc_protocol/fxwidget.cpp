...
void FxWidget::OnButtonClick_Plc_Connect() {
  if (plc_->FxIsConnected()) {
    ...
    if (UserInputAddress())
    plc_->FxConnectToPlc(plc_ -> plc_address_, plc_ -> plc_server_port_);
  }
}
...
bool FxWidget::UserInputAddress() {
  ...
  data["PLC Address"] = plc_ -> plc_address_;
  data["Server port"] = plc_ -> plc_server_port_;
  ...
  while(true) {
    if(InputFormDialog::getInput("Enter PLC Address", data, options)) {
      ...
      if(h_address.protocol() == QAbstractSocket::IPv4Protocol) {
        plc_ -> plc_address_ = data.at<QString>("PLC Address");
        plc_ -> plc_server_port_ = data.at<int>("Server port");
        return true;
      }
    } else {
      return false;
...
void FxWidget::PlcConnectedHandle() {
  ...
  ui->label_plc_ip_address->setText(plc_-> plc_address_);
  ui->label_plc_server_port->setText(QString::number(plc_-> plc_server_port_,10));
...
void FxWidget::AnimateClickButtonInit(){
  ui->btn_add_device->animateClick();
}
