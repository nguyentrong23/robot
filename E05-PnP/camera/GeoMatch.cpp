...
bool GeoMatch::addGeoMatchModel(String pathTemplate, String modelName, Point pick) {
    GeoModel tempModel(pathTemplate, modelName);
    if (tempModel.isImageEmpty()) {
        return false;
    }
    tempModel.modelLearnPattern();
    tempModel.pickPosition = pick;
    ModelSrc.push_back(tempModel);
    return true;
}
...
//        break;
  ...
  int maxModelSize = 0;
  for(int counter=0;counter<ModelSrc.size();counter++) {
  ...
  matchingResult.isAreaLessThanLimits = (sumArea < maxModelSize*NUM_MODEL_FOR_SUM_AREA) ? true : false;
}
