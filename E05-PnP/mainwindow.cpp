...
//    ui->statusbar->showMessage(content);
  ...
  connect(pnp_controller_, &PnpController::PnpSignal_DhrSetValue,
          this, [this]{dhr_SetValue();
  });
}
...
void MainWindow::connectUiEvent() {
  ...
  /// BUTTON EVENTS
  connect(ui->btn_dashboard_all_connect, &QPushButton::clicked,
          this, &MainWindow::on_Click_Dashboard_AllConnect);
  connect(ui->btn_dashboard_all_disconnect, &QPushButton::clicked,
          this, &MainWindow::on_Click_Dashboard_AllDisConnect);
  ...
  connect(ui->btn_dashboard_counter_reset, &QPushButton::clicked,
          this, &MainWindow::on_Click_Dashboard_Counter_Reset);
...
void MainWindow::dhr_UiInitialize() {
  ...
  connect(dhController, &DHController::DHSignal_Connected, this, [this] () {
    ...
    if (ui->widget_Lce_Control->IsAutoInit()) {
/*      dhController->DH_AddFuncToQueue(DH_LCE::SetInitDevice(lce_address));*/
      dhController->DH_AddFuncToQueue(DH_LCE::SetLinearPosition(lce_address,0));
      dhController->DH_AddFuncToQueue(DH_LCE::SetLinearForce(lce_address,100));
      dhController->DH_AddFuncToQueue(DH_LCE::SetLinearSpeed(lce_address,100));
    }
  ...
  connect(dhController, &DHController::DHSignal_Disconnected, this, [this] () {
    ...
    ui->widget_Lce_Control->SetSlaveEditBoxEnable(true);
    ui->btn_dhr_serial_connect->setEnabled(true);
  });
...
//  });
  ...
  connect(ui->widget_Lce_Control, &DhLceWidget::SignalsLceInitialize, this, [this] () {
      dhController->DH_AddFuncToQueue(DH_LCE::SetInitDevice(lce_address));
  });

  connect(ui->widget_Lce_Control, &DhLceWidget::SignalsLinear_PositionEdited,
          this, [this] (int value) {
              dhController->DH_AddFuncToQueue(
                  DH_LCE::SetLinearPosition(lce_address, value));
          });

  connect(ui->widget_Lce_Control, &DhLceWidget::SignalsLinear_ForceEdited,
          this, [this] (int value) {
              dhController->DH_AddFuncToQueue(
                  DH_LCE::SetLinearForce(lce_address, value));
          });

  connect(ui->widget_Lce_Control, &DhLceWidget::SignalsLinear_SpeedEdited,
          this, [this] (int value) {
              dhController->DH_AddFuncToQueue(
                  DH_LCE::SetLinearSpeed(lce_address, value));
          });
}
void MainWindow::dhr_SetValue(){
    dhController->DH_AddFuncToQueue(DH_PGC::SetGripperForce(pgc_address, 20));
    dhController->DH_AddFuncToQueue(DH_RGI::SetGripperForce(rgi_address, 50));
    dhController->DH_AddFuncToQueue(DH_LCE::SetLinearPosition(lce_address, 0));
}
...
void MainWindow::dhr_DisplayRgiInfo() {
  ...
  if (!dhController->DH_GetLceData(lce_address, lce_data)) {
    this->statusBar()->showMessage("Get device info fail");
    return;
  }
  ...
  ui->widget_Lce_Control->ShowLceDeviceInfo(lce_data.feedback);
}
...
//////////  BUTTON ACTIONS
void MainWindow::on_Click_Dashboard_AllConnect() {
    ...
    if(!dhController->DH_IsConnected()) {
        ...
        connect(serial_dialog, &SerialSettingDialog::UserAcceptSerialSetting,
                this, [this] (SerialSetting setting) {
                    ...
                    lce_address = ui->widget_Lce_Control->GetSlaveAddress();
                    ui->widget_Lce_Control->SetSlaveEditBoxEnable(false);
                    ...
                    dhController->DH_SetLceAddress(lce_address);
                    dhController->DH_Connect(setting);
                });
        serial_dialog->ShowDialog();
    }
/***
    if (fx_plc->FxIsConnected()) {
        fx_plc->FxDisconnectFromPlc();
    } else {
        fx_plc->FxConnectToPlc(fx_plc->FxPlcGetIpAddress(), fx_plc->FxPlcGetPort());
    }
***/
    if (!fx_plc->FxIsConnected()) {
        fx_plc->FxConnectToPlc(fx_plc->FxPlcGetIpAddress(), fx_plc->FxPlcGetPort());
    ...
    CDeviceInfo cameraInfo = cameraControl->getDeviceInfo();
    qDebug() << "Camera Ip Address: " << cameraInfo.GetAddress();
    qDebug() << "DH Robotic Port: " << serial_port_name;
    qDebug() << "Plate Ip Address: " << plateAddress << ":" << plateServerPort;
    qDebug() << "FX PLC Ip Address: " << fx_plc->FxPlcGetIpAddress()<< ":" << fx_plc->FxPlcGetPort();
    qDebug() << "Cobot Han's Ip Address: " << robotAddress;
}

void MainWindow::on_Click_Dashboard_AllDisConnect() {

    QMessageBox::StandardButton reply;
    reply = QMessageBox::question(nullptr, "Confirm", "Do you want to disconnect all devices?",
                                  QMessageBox::Yes|QMessageBox::No);
    if (reply == QMessageBox::Yes) {
        if(flexPlate->isFeederConnected()) {
            flexPlate->FeederDisconnected();
        }
...
void MainWindow::on_Click_Dashboard_Counter_Reset() {
    QMessageBox::StandardButton reply;
    reply = QMessageBox::question(nullptr, "Confirm", "Do you want reset total object picked?",
                                  QMessageBox::Yes|QMessageBox::No);
    if (reply == QMessageBox::Yes) {
        pnp_controller_ -> setTotalObjectPicked(0);
    }
}
 ...
 *  */
  QString sampleName = filePath.split("/").last();
...
void MainWindow::on_ViewList_CurrentRowChanged_Model(int currentRow) {
  ...
  qDebug()<<"currentRow" << ui->list_Model_ViewList->currentRow();
  vector<ImageMatch::GeoModel> modelSrc = matcher->getModelSource();
...
void MainWindow::on_Click_Setting_Load() {
    ...
    /// update viewlist
    matcher->clearMatchModel();

    for (const auto& pair : setting.mlist.Model_dic)
    {
        QString label = pair.first;
        QString path = pair.second.first;
        Point pick_point = pair.second.second;
        if(path.isEmpty() || label.isEmpty())
        {
            continue;
        }
        matcher->addGeoMatchModel(path.toStdString(),label.toStdString(), pick_point);
        model_UpdateViewList();
        int lastIndex = ui->list_Model_ViewList->count() - 1;
        ui->list_Model_ViewList->setCurrentRow(lastIndex);
        model_UpdateTemplateViewInfo();
    }
    int temp = setting.lang_control.lang_curr;
    currentLanguage = static_cast<LanguagesUI>(temp);
    ui_language_reload();

    pnp_controller_ -> setTotalObjectPicked(setting.systemConfig.LastState.totalObjectPicked);
    plateAddress = setting.systemConfig.VibratingPlateConfig.ipAddress;
    plateServerPort = setting.systemConfig.VibratingPlateConfig.port;
    robotAddress = setting.systemConfig.RobotConfig.ipAddress;

    fx_plc->FxPlcSetIpAddress(setting.systemConfig.FxPlcConfig.ipAddress);
    fx_plc->FxPlcSetPort(setting.systemConfig.FxPlcConfig.port);
...
void MainWindow::on_Click_Setting_Save() {
  ...
  SettingHandler setting;
  setting.mlist.Listmodel = matcher->getModelSource();
  ...
  setting.lang_control.lang_curr = currentLanguage;
  setting.systemConfig.LastState.totalObjectPicked = pnp_controller_ ->getTotalObjectPicked();
  setting.systemConfig.VibratingPlateConfig.ipAddress = plateAddress;
  setting.systemConfig.VibratingPlateConfig.port = plateServerPort;
  setting.systemConfig.RobotConfig.ipAddress = robotAddress;

  setting.systemConfig.FxPlcConfig.ipAddress = fx_plc->FxPlcGetIpAddress();
  setting.systemConfig.FxPlcConfig.port = fx_plc->FxPlcGetPort();

  setting.save(fullPath);

}

void MainWindow::ui_language_reload() {
    QString translatorPath = QApplication::applicationDirPath();
    bool loadlangState = false;

    switch (currentLanguage) {
    case LanguagesUI::VN:
        translatorPath.append("/E05-PnP_vi_VN.qm");
        loadlangState = uiTranslator.load(translatorPath);
        qApp->installTranslator(&uiTranslator);
    ...
    case LanguagesUI::JP:
        ...
        break;
    case LanguagesUI::EN:
        loadlangState = qApp->removeTranslator(&uiTranslator);
        ui->comboBox_Language->setCurrentIndex(0);
        break;
    }
    if(loadlangState)
    {   ui->retranslateUi(this);
        qDebug()<<"loaded language";
    }
}
