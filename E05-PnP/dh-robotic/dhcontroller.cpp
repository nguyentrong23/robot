...
void DHController::DH_SetLceAddress(int address) {
  device_lce_address_ = address;
}
...
bool DHController::DH_GetLceData(int address, LCEData &device) {
  if ((device_lce == nullptr) || (device_lce->slave_address_ != address)) {
    return false;
  }
  device = device_lce->DeviceInfo();
  return true;
}
...
void DHController::ModbusHodlingRegsResponse(const int slave_address,
  ...
  if(slave_address == device_lce->slave_address_) {
    device_lce->UpdateData(unit);
    return;
  }
}
...
void DHController::ModbusSendFunction(ModbusFunc func_code) {
  ...
  if ((func_code.slave_address != device_pgc_address_) &&
      ...
      (func_code.slave_address != device_lce_address_)) {
    return;
  }
...
bool DHController::DhDeviceInit() {
  if (device_pgc_address_ == device_rgi_address_ || device_pgc_address_ == device_lce_address_ || device_rgi_address_ == device_lce_address_ ) {
    return false;
  }

  device_pgc = new DH_PGC(device_pgc_address_);
  device_rgi = new DH_RGI(device_rgi_address_);
  device_lce = new DH_LCE(device_lce_address_);
  return true;
}
...
void DHController::DhDeviceCollectInfo() {
  ...
  ModbusSendFunction(device_lce->GetDeviceFeedbackInfo());
  ...
  DhLinearStatus lce_grip_state;
  if (device_lce->IsLinearStateChange(lce_grip_state)) {
    emit DHSignal_LceLinearStateChanged(lce_grip_state);
  }
